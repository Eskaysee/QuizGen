...
public class RedBlackTree<T extends Comparable<? super T>> {
    ...
    public void deleteTree(){root = nil;}

    public void delete(T key) {
        delete(new RedBlackNode<T>(key));
    }
    ...
    public void delete(RedBlackNode<T> v){
        ...
        // if either one of z's children is nil, then we must delete z
        if (isNil(z.left) || isNil(z.right))
            y = z;

            // else we must delete the successor of z
    ...
    // @param: x, the child of the deleted node from delete(RedBlackNode v)
    // Restores the Red Black properties that may have been violated during
    // the removal of a node in delete(RedBlackNode v)
    ...
    void printTree(RedBlackNode<T> node){
        ...
        printTree(node.left);
        if(node == root) {
            System.out.print(((node.color == 1) ? "Color: Red " : "Color: Black ") + "Key: " + node.key + " ROOT\n");
            if(node.left != nil){
            ...
            System.out.print(((node.color == 1) ? "Color: Red " : "Color: Black ") + "Key: " + node.key + "\n");
            if(node.left != nil){
                System.out.println("    Left Child-> " + node.left.key);
            }
            if(node.right != nil){
    ...
    public int getSize ( RedBlackNode<T> node )
    {
        if (node == nil)
            return 0;
        else
    ...
    public void visit ( RedBlackNode<T> node )
    {
        path.add(node.key);
    }
    ...
    public void setLeaves(RedBlackNode<T> node, ArrayList<T> allLeaves){
        if (node != nil){
            if (node.right == nil && node.left == nil) {
                allLeaves.add(node.key);
                node.isLeaf = true;
                return;
            }
    ...
    public void preOrder ( RedBlackNode<T> node )
    {
        if (node != nil)
        {
            visit (node);
    ...
    public void postOrder ( RedBlackNode<T> node )
    {
        if (node != nil)
        {
    ...
    public void clearTree(){root = nil;}

    public class RBTQueueNode<T extends Comparable<? super T>>
    {
        RedBlackNode<T> node;
        RBTQueueNode<T> next;

        public RBTQueueNode ( RedBlackNode<T> n, RBTQueueNode<T> nxt )
        {
            node = n;
            next = nxt;
        }
    ...
    public class RBTQueue<T extends Comparable<? super T>>
    {
        RBTQueueNode<T> head;
        RBTQueueNode<T> tail;
        ...
        public RedBlackNode<T> getNext ()
        {
            if (head == null)
                return null;
            RBTQueueNode<T> qnode = head;
            head = head.next;
            if ( head == null )
                tail = null;
            return qnode.node;
        ...
        public void enQueue ( RedBlackNode<T> node )
        {
            if (tail == null)
            {
                tail = new RBTQueueNode<T> (node, null);
                head = tail;
            }
            else
            {
                tail.next = new RBTQueueNode<T> (node, null);
                tail = tail.next;
            }
        }
    }
